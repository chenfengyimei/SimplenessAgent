...
func TestSegmentPlannerRecoversWhenFirstReplyIsEmpty(t *testing.T) {
	provider := &plannerScriptProvider{responses: []string{
		"",
		`{"summary":"inspect first","terminal_segment":true,"steps":[{"ref":"scan","title":"Scan files","goal":"Locate the implementation","dependencies":[],"tool_intents":["list_files"],"acceptance_intent":"Relevant files are identified"}]}`,
	}}
	planner, err := NewSegmentPlanner(provider)
	if err != nil {
		t.Fatal(err)
	...
	if err != nil || len(result.Steps) != 1 {
		t.Fatalf("empty-reply recovery did not succeed: plan=%#v err=%v", result, err)
	}
	...
	if !strings.Contains(repair, "no answer text") || !strings.Contains(repair, "Do not reason, think, or write any preamble") {
		t.Fatalf("empty-response repair lost its anti-thinking instruction: %s", repair)
	}
}

func TestSegmentPlannerReportsTokenExhaustionDiagnosisForEmptyReplies(t *testing.T) {
	provider := mock.Provider{Response: ""}
	planner, _ := NewSegmentPlanner(provider)
	now := time.Now().UTC()
	horizonPlan := DefaultPlan("task", now)
	...
	if err == nil || !strings.Contains(err.Error(), "finish_reason") || !strings.Contains(err.Error(), "hidden reasoning") {
		t.Fatalf("empty response error lost its diagnosis: %v", err)
	}
}
